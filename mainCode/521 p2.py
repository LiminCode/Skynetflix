...
def get_date(date_string,
	...
	try:
		...
		if len(r)==1: return dt.date(r[0],1,1)
		if len(r)==2: return dt.date(r[1],r[0],1)
...
def filter_return_count(f, values)
	...
	if count:
		try:
			...
			raise ValueError(f'get highest rated movies: enter a valid count (you entered {count})')
		if count<1:
			raise ValueError('get highest rated movies: enter a count > 0 (you entered {count})')
		values.append(str(count)) # this will be passed to the parameterized query
...
def add_movie(conn, *, id_parse = ACTOR_ID_PARSE, info_cap=MAX_INFO_SIZE):
	...
	date = custom_select("Enter release date (empty field sets date to today)", get_date)[1]
	if not date:
		date = dt.date.today()
